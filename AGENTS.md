...
The design system's tokens live in the Tailwind v4 preset (`tailwind/theme.css`, in an `@theme` block): a restricted spacing scale, custom shadows/radii, and colors. Consumers pull the whole preset in with a single `@import '@archilogic/honeycomb/tailwind'` — there is no separate CSS bundle to import. Component styles that can't be expressed as utility classes (e.g. the Checkbox box, `SelectableOptionGroup`'s `:has()` highlight) live in `tailwind/components.css` under `@layer components`, not in the component's `<style>` block. Caveat: `@layer components` loses to `@layer utilities` on the same element (layer order is `theme, base, components, utilities`), so a rule that must override a utility — e.g. un-hiding the check mark over its `hidden` class — has to be a `peer-*`/variant utility in the template, not a component-layer rule. Colors support theming via CSS variables — `tailwind/variables.css` defines `--honeycomb-color-*` custom properties (defaults on `:root`), `colorMap.json` references them, and `src/colors/` exports the variable names for programmatic use. There are two copies of `colorMap.json` (`tailwind/` and `src/colors/`) — keep them in sync when changing colors. Prettier runs with `prettier-plugin-tailwindcss`, so class order is enforced.
